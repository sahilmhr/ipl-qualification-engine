...
export function SimulatorTab({
  ...
  return (
    <div>
      ...
      >
        <div style={{ fontSize: 10, color: C, letterSpacing: 2, marginBottom: 6 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10, flexWrap: "wrap" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
          <button
            onClick={() => setScenario(buildBestScenario(simTeam, fixtures))}
            style={{
              padding: "5px 11px",
              borderRadius: 6,
          ...
          >
            ...
            {ranked.map((team, idx) => {
              ...
              return (
                ...
                >
                  <span style={{ width: 16, fontSize: 12, fontWeight: 700, color: inTop4 ? C : "var(--color-text-secondary)" }}>
                    {idx + 1}
                  </span>
                  <span style={{ width: 7, height: 7, borderRadius: "50%", background: team.color, flexShrink: 0 }}></span>
                  <span style={{ flex: 1, fontSize: 11, fontWeight: inTop4 ? 600 : 400, color: "var(--color-text-primary)" }}>
                    {team.short}
                  </span>
                  <span style={{ fontSize: 11, fontWeight: 700, color: G }}> {s.wins}W </span>
                  <span style={{ fontSize: 11, color: "#FBBF24", fontWeight: 600 }}> {s.points}pts </span>
                  {changed && (
                    <span style={{ fontSize: 9, color: C, background: `${C}22`, padding: "1px 4px", borderRadius: 3 }}>
                      sim
                    </span>
                  )}
                </div>
      ...
      {(() => {
        ...
        return (
          ...
          >
            <div style={{ fontSize: 10, color: "var(--color-text-secondary)", marginBottom: 4, letterSpacing: 2 }}>
              {team.short} — SIMULATED QUAL ({count} hypothetical result{count !== 1 ? "s" : ""} applied)
            </div>
            <div style={{ fontSize: 14, fontWeight: 700, color: vc }}>
              {simQ.impossible
